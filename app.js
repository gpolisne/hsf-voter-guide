    ...
    fetch("data.json?v=" + Date.now())
      ...
      .then(function (json) {
        ...
        boot();
            ...
            .catch(function (err) {
        var detail = "Technical detail: " + esc(err.message);

        if (window.location.protocol === "file:") {
          showStatus(
            "Open the guide through a web server",
            "Your browser blocks a page opened straight from a folder from " +
              "reading data.json. This is a browser security rule, not a " +
              "problem with the guide or the file. " +
              "Open a terminal in this folder, run " +
              "<strong>python -m http.server 8000</strong>, then visit " +
              "<strong>http://localhost:8000</strong> instead. " + detail
          );
        } else if (err instanceof SyntaxError) {
          showStatus(
            "The guide could not load",
            "<strong>data.json</strong> has a formatting error, usually a " +
              "missing comma or quote mark. If it was generated by " +
              "build_data.py, run that again. If it was edited by hand, " +
              "paste it into jsonlint.com to find the line. " + detail
          );
        } else {
          showStatus(
            "The guide could not load",
            "<strong>data.json</strong> could not be read. It may be missing " +
              "from this folder, or the site may still be finishing its " +
              "deploy. Wait a minute and refresh. " + detail
          );
        }
      });
  }
